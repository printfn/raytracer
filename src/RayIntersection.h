 ...
 *
 * RayIntersections can also be sorted on distance along the ray.
 */
...
public:
    ...
    double distance = 0.0; //!< The distance along the Ray to the intersection Point.
    bool entersObject = false; //!< Whether the ray would enter or exit the object, needed for refraction
     ...
     */
    bool operator<(const RayIntersection &ri) const {
